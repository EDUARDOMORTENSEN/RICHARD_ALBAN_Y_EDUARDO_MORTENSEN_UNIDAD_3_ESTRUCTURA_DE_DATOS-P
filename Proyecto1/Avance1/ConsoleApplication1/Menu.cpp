...
void Menu::navigation()
{
    ...
    {
        if (i == 0) {
            pixel_boxes.push_back(ESQUINA);
            pixel_boxes.push_back(ESQUINA + ANCHO);
            j = 1;
        }
        else {
            pixel_boxes.push_back(pixel_boxes.at(j-1) + 1);
            pixel_boxes.push_back(pixel_boxes.at(j) +  ANCHO);
            j++;
        }
    }
...
bool Menu::usarMouse(size_t* i) {
    ...
    if (GetCursorPos(&cursor_pos))
    {
        ...
        {
            for (size_t i = 0; i < pixel_boxes.size(); i += 2)
            {
                if (cursor_pos.y >= pixel_boxes.at(i) &&
                    cursor_pos.y <= pixel_boxes.at(i + 1) &&
